...
With the use of prop getters we can compose the props in a way that allows for
users of our API to add their own functionality, while at the same time getting
the enhanced capabilities of our own prop getter.

In other words: We can have default values, and let the user build upon those
default values.

Prop Collections and Prop getter can be used in function components as well,
but it is in custom hooks they really shine.
## Background
