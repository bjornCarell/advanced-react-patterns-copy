...
We abstract the logic, that goes in to the dispatch functions, from the
component to a separate function that takes the dispatch as an argument(and
whatever other arguments needed). In this way the users of our API can just
consume that function, instead of having to implement the dispatch each time.
